...
def get_clusterplots(km_pca_df,model,figloc,filename,feat1,feat2):
    
    ax = plt.subplots(figsize=(10,10))
 
    #ax = sns.scatterplot(x=km_pca_df.iloc[:,0], y=km_pca_df.iloc[:,1], data=km_pca_df,hue = 'AnomalyInd')
    ax = sns.scatterplot(x=km_pca_df[feat1], y=km_pca_df[feat2], data=km_pca_df,hue = 'AnomalyInd')
    ax.set_title(model + ' - Anomaly Detection')
  
    ax.figure.savefig(figloc + str(filename) + '.png')
    return ax

def get_clusterPCAAnomalyplots (km_pca_df,x_clust_center,model,figloc,filename):
    
    plt.figure(figsize=(10, 8))

    colors = {0:'blue', 1:'red'}
    plt.scatter(km_pca_df[1], km_pca_df[2], c=km_pca_df['ClusterNumber'], s=50, cmap='viridis')
    plt.scatter(x_clust_center[:,0], x_clust_center[:,1], marker='*', c='r', s=150)
    
    plt.title('K-Means Clustering')
    plt.xlabel('principal component 1')
    plt.ylabel('principal component 2')
    
    plt.title(model + ' - K-Means PCA Clusters')
    plt.savefig(figloc + str(filename) + '.png')
    
    return True
   


def get_Autoencoderplot(history,figloc,filename = 'AutoEncoderLossPlot',model = 'Auto-Encoder'):
    
   #history = auto_model
   #figloc = figloc 
   #filename = 'AutoEncoderLossPlot'
   #model = 'Auto-Encoder'
...
def get_ReconsutructionErrorsplot(error_df,threshold_fixed,figloc,filename = 'AutoEncoder Reconstruction Loss'):
    fig, ax = plt.subplots()
    #ax = plt.subplots(figsize=(10,10))
    ax.plot(error_df['Reconstruction_error'], linewidth=2, label='Reconstruction_error')
    ax.hlines(threshold_fixed, xmin=1, xmax=error_df.shape[0], colors="r", label='Threshold - ' + str(threshold_fixed))
    ax.hlines(threshold_fixed, get_xlim()[0], get_xlim()[1], colors="r", zorder=100, label='Threshold')
...
def get_ConfusionMatrixplot(y_Actual, y_Pred,figloc, filename):
    
    ax = plt.subplots(figsize=(10,10))
    
    data = {'y_Predicted': y_Pred.ravel(),
            'y_Actual':    y_Actual
            }
    ...
    ax.set_title(filename + ' - Anomaly Detection')
    ax.figure.savefig(figloc + str(filename) + '.png')
 
    return ax

def get_GANSTraningLossesPlot(losses,figloc, filename):
...
def get_BIGANSTraningLossesPlot(losses,figloc, filename):
#Plot the Training Losses
    plt.subplots(figsize=(10,10))
    
    losses = np.array(losses)
    plt.plot(losses.T[0], label='Generator')
    plt.plot(losses.T[1], label='Encoder')
